...
# Caleb Yu
#
# Converted to a module to be imported from a Jupyter notebook by: 
...
def jarm_worker(q, pbar=None):
    while True:
        ...
        # Add to master domain dictionary
        if ip != None:
            result_list.append([dhost,ip,result])
        time.sleep(0.1)
        q.task_done()
        # Increase the progress bar counter if the widget object was received from the Jupyter notebook
        if pbar is not None:
            pbar.value += 1


# The default destination port for the analysis
destination_port = 443
# This list will collect the JARM hashes from the worker threads
result_list = []
